...
export class WebCEOComponent implements OnInit {
  ...
  getCountryName(code){
    let result=code;
    if(this.country_list.hasOwnProperty(code)){
      result=this.country_list[code];
    }
    return result;
  }
  ...
  setWebCEOData(){
    let top_keyword=[];
    let keyword=[];
    this.webceo.getProject(this.rankingAccount).then((result)=>{
      var count=0;
      result[0]["data"]["ranking_data"].forEach((item)=>{
        count++;
        ...
        try{
          ...
          var obj_desktop={"key":item.kw,"organic_data":current_pos,
            ...
            "location":this.getCountryName(item.positions[0].country),
