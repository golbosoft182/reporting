...
export class ClientComponent implements OnInit {
  ...
  public current_goal_total;
  public compare_goal_total;
  ...
  constructor(private route: ActivatedRoute,private location: Location,private client_service:ClientService,private helper:Helper,private chart:ChartService){
      ...
      // });
    this.webCEOData["top_keyword"]=[];
    this.webCEOData["keyword"]=[];
    this.webComment["topkeyword"]="";
    this.webComment["topsearch"]="";

    this.current_goal_total=0;
    this.compare_goal_total=0;
  }
  ...
  ngOnInit(): void {
     this.route.params.subscribe((params: Params) => {
        ...
        this.client_service.getAllData(id).then((res)=>{
          ...
          this.webCEOData["top_keyword"]=this.data.current["top_keyword"];
          this.webCEOData["keyword"]=this.data.current["keyword"];
          this.webComment["topkeyword"]=this.comment["topkeyword"];
          this.webComment["topsearch"]=this.comment["topsearch"];

          //Ubqunioal custom event
          ...
          if(this.goalChannelUpdate.length>0){
            this.goalViaChannelUpdate(this.goalChannelUpdate[0].id);
          }
  ...
  siteEventUpdate(value){
    ...
    if(this.client_data["client_name"]==="Ubiquinol"){
      this.compare_goal_total=0;
      this.current_goal_total=0;
      this.events.map((event)=>{
        this.compare_goal_total+=parseInt(event.compare);
        this.current_goal_total+=parseInt(event.current);
      });
      this.current_goal_total+=parseInt(this.data.current.goalEvents.filter(goal=>goal[0].name==="goal"+2+"Completions")[0][2]);
      this.compare_goal_total+=parseInt(this.data.compare.goalEvent.filter(goal=>goal[0].name==="goal"+2+"Completions")[0][2]);
    }
  }
  ...
  goalViaChannelUpdate(id){
    // console.log(this.data["listofGoalSelection"]);
    if(this.client_data["client_name"]==="Viadux"){
      this.goalChannelUpdate=this.goalChannelUpdate.filter((goal)=>{
        console.log(goal);
        return goal;
      });
    }
    console.log(this.data.current.goalEvents.filter(goal=>goal[0].name==="goal"+id+"Completions"));
